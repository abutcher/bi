  ...
  (let* ((id      0)
    ...
    (labels ((min-max (row) (mapc #'range-update row ranges))
      ...
      (setf (cluster-rows tbl)
            (vector! (mapcar #'row0 data)))
      (let* ((quads (make-quads (cluster-rows tbl))))
	(setf (cluster-quad1 tbl)
	      (construct-cluster head (first quads)))
	(setf (cluster-quad2 tbl)
	      (construct-cluster head (second quads)))
	(setf (cluster-quad3 tbl)
	      (construct-cluster head (third quads)))
	(setf (cluster-quad4 tbl)
	      (construct-cluster head (fourth quads))))
